...
<div class="w-full h-5/12 relative mb-4 overflow-hidden">
    @if ($item['discount'] > 0)
        <span class="absolute top-2 left-2 rounded-full bg-gray-300 py-1 px-2 text-sm font-extralight">{{ $item['discount'] }}%</span>
    @endif
...
<div class="w-full h-5/12 px-6 flex flex-col justify-between items-start">
    <span class="rounded-full bg-gray-300 py-1 px-2 text-sm font-extralight">{{ $item['category'] }}</span>
    <div class="my-4">
        <h3 class="text-2xl font-bold">{{ $item['title'] }}</h3>
        <p>{{ $item['description'] }}</p>
    </div>
    <span class="rounded-full bg-gray-300 py-1 px-2 text-sm font-extralight">{{ $item['stock'] }} unidades</span>
</div>
