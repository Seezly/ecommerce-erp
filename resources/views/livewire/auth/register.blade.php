...
<x-layouts::auth :title="__('Regístrate')">
	<div class="flex flex-col gap-6">
		...
		<form method="POST" action="{{ route('register.store') }}" class="flex flex-col gap-6">
			@csrf
			<!-- Name -->
			<flux:input name="name" :label="__('Nombre completo')" :value="old('name')" type="text" required autofocus
			...
			<!-- Phone Number -->
			<flux:input name="phone-number" :label="__('Número de teléfono')" :value="old('phone-number')" type="text"
				required autocomplete="phone-number" placeholder="+58 (424) 541-2882" />
